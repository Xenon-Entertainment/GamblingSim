...
class Results:
    ...
    def output_balance_and_win(self, player: Person):
        print(f"\nWinner? : {self.check_win()}")
        _change = 1000 if self.check_win() else -10
        print(f"\nBalance: {player.balance} -> {player.balance + _change}")
        player.balance += _change
